    ...
    <Container>
      <ExploreContentWrapper>
        <div className='header-wrapper'>
          <img className='arrow-image' src={ArrowIcon} alt="ArrowIcon" />
          <h2><img src={HeadingIcon} alt="HeadingIcon" /> Explore Full-suite of Prep, Pack,
            and Ship Features</h2>
        </div>
        <div className='card-items-wrapper'>
          {Data.map((item, index) => (
            <Card index={index} CardIcon={item.icon} text={item.text} heading={item.heading} />
          ))}
        </div>
